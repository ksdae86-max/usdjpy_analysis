...
void OnTick() {
    ...
    // データ取得
    double rsi[1], up[1], lw[1], mid[1], adx[1], atr[1];
    ...
    if(!IsValidValue(bid) || !IsValidValue(up[0])) return;

    string msg = "";
    double atr_pips = (atr[0] / (SymbolInfoDouble(TARGET_SYMBOL, SYMBOL_POINT) * 10.0));
    double bb_width = (up[0] - lw[0]) / (SymbolInfoDouble(TARGET_SYMBOL, SYMBOL_POINT) * 10.0);

    // --- ロジック1：中心線（ミドル）通知 ---
    if(MathAbs(bid - mid[0]) <= (InpMidBuffer * SymbolInfoDouble(TARGET_SYMBOL, SYMBOL_POINT) * 10.0)) {
        msg += StringFormat("\n⚓【ミドル付近】価格が中心線(%.3f)に接近", mid[0]);
    }

    // --- ロジック2：過熱相場の切り捨て（最優先） ---
    if(atr_pips >= InpPanicATR) {
        msg += StringFormat("\n⚠️【高ボラ静観】ATR:%.1fp (継続性低リスク)", atr_pips);
    } 
    else {
        // --- ロジック3：スクイーズ＆適正ボラでのブレイク ---
        if(bb_width <= InpSqueezePips) {
            if(bid > up[0] && adx[0] >= InpADXMin) msg += "\n🚀【順張り好機】適度な勢いで上抜け！";
            else if(bid < lw[0] && adx[0] >= InpADXMin) msg += "\n📉【順張り好機】適度な勢いで下抜け！";
            else msg += StringFormat("\n💎【スクイーズ】幅:%.1fp (待機中)", bb_width);
        }
